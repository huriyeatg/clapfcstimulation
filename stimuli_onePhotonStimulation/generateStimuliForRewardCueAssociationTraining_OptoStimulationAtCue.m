...
tic
%% parameters
params.savePath = fullfile(root_path,'onePhotonStimulation','stimuli_onePhotonStimulation',['StimuliForTraining_OptoAtCue',date]);
...
trial_stimSignal = [trial_stimSignal(1:end-1),zeros(1,(params.rewardDuration*fs +1 ))];
...
for k=1:(params.nTrialsCueReward + params.nTrialsCueRewardStim)
    ...
    if ismember(k, params.trialIDforStimTrials) % means no reward 
